...
contract Voting {
    ...
    // 🆕 hasVoted theo từng cuộc bầu cử
    mapping(uint => mapping(address => bool)) public votedInElection;
    
    uint    public candidatesCount;
    uint    public electionId;      // 🆕 ID cuộc bầu cử
    ...
    function startElection(uint _durationSeconds) public onlyOwner {
        ...
        // 🆕 Reset vote count + tăng electionId
    ...
    function vote(uint _candidateId) public electionOpen {
        // 🆕 Check theo electionId hiện tại
        require(!votedInElection[electionId][msg.sender], "Ban da bo phieu roi!");
        ...
        votedInElection[electionId][msg.sender] = true;
        candidates[_candidateId].voteCount++;
        emit Voted(msg.sender, _candidateId);
    }

    // 🆕 Check đã vote trong cuộc bầu cử hiện tại chưa
    function hasVoted(address _voter) public view returns (bool) {
        return votedInElection[electionId][_voter];
    }
    ...
    function getElectionInfo() public view returns (
        ...
        _isActive = isActive;
        _deadline = deadline;
        _timeLeft = (isActive && block.timestamp < deadline)
